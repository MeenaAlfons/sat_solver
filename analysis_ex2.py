...
if False:
    for metric in metrics:
        plt.plot('numOfConstraints', metric, data=data, linestyle='', marker='o', markersize=0.8)
        plt.ylabel(metric + 's')
        plt.xlabel('number of constraints')
        plt.show()
...
if False:
    for metric in metrics:
        plt.hist2d(data[data[metric]<3000]['numOfConstraints'], data[data[metric]<3000][metric], bins=[27, 70], cmap=plt.cm.Reds)
        plt.ylim(0, 3000)
        plt.colorbar()
        plt.show()
